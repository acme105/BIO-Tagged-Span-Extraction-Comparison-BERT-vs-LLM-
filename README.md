...
- [jjzha/sayfullina](https://huggingface.co/datasets/jjzha/sayfullina) (Soft skills focused)
- [jjzha/skillspan](https://huggingface.co/datasets/jjzha/skillspan)
- **green**
- **gnehm**
...
1. **Baselines**: Few-shot prompting and Retrieval-Augmented Generation (RAG).
2. **SFT (Supervised Fine-Tuning)**: Fine-tuning Qwen2.5-3B-Instruct using LoRA.
3. **SFT + ICL (In-Context Learning)**: Enhancing SFT with dataset-specific in-context demonstrations.
4. **SRICL (Self-Reflective In-Context Learning)**: Combining SFT with self-reflective reasoning and in-context examples for superior NER performance.
...
Training scripts are optimized for running on GPUs (e.g., Kaggle T4 x2 or Google Colab) using libraries like `bitsandbytes` and `unsloth` for efficient parameter fine-tuning.
